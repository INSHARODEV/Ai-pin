...
export abstract class BaseRepository<T  > implements BaseRepositoryInterface<T> {
  ...
  this.logger.warn(`${JSON.stringify(data)} is bieng created `)
    ...
    this.logger.log(`Successfully created document: ${JSON.stringify(createdDocument)}`);
...
async findOneById(id: string): Promise<unknown | Error> {
  ...
  this.logger.verbose(`attemting to get user with its id ${id.slice(1,5) }...`)
...
async findOne(options: FilterQuery<T>,fileds:string=''): Promise<unknown | null> {
  this.logger.debug(`geting the ${this.model.name} uisng data =>${JSON.stringify(options)}`)  
...
async find({limit, skip, queryStr, fields, sort, page }) :Promise<PaginatedData> {
 ...
 async findOneAndUpdate(filter: FilterQuery<T>, updateData: Partial<T>): Promise<unknown | null> {
  ...
  async updateOne(id: MongoDbId, updateData: Partial<T>): Promise<unknown | null> {
    this.logger.debug(`updating  model ${this.model.name } , a document that as the id of ${id} with data: ${JSON.stringify(updateData)}`)
...
this.logger.error(`faild to update document  with error status :${errror.status} and stack :${errror.stack}`);
